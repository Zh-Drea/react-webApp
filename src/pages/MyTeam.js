...
class MyTeam extends BaseComponent{
								...
								return (
									<div key={index} style={{padding:15,borderRadius:10,backgroundColor:"white",marginBottom:index==thiz.state.records.length-1?0:10,}}>
										<div>
											...
											<div style={{display:"inline-block",marginLeft:10,verticalAlign:"middle",fontSize:18,}}>
												<p>{item.nickname?item.nickname:""}<span style={{display:item.userType&&item.userType.indexOf("VIP")>=0?"inline-block":"none",fontSize:13,color:"#FF9529",
											backgroundColor:"#1F1F1F",height:15,paddingLeft:8,paddingRight:8,borderRadius:8,marginLeft:15,fontStyle:"italic"}}>{item.userType&&item.userType.indexOf("VIP")>=0?item.userType:""}</span></p>
...
MyTeam = BaseComponent.connect(BaseComponent.mapStateToProps, BaseComponent.mapDispatchToProps)(MyTeam);
